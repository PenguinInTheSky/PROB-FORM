...
if __name__ == "__main__":
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            with torch.no_grad():
                action = torch.zeros((args.num_envs,) + envs.single_action_space.shape).to(device)
                for i in range(args.num_envs):
                    act, logprob, _, value = agents[int(next_rm_state[i].item())].get_action_and_value(next_obs[i].unsqueeze(0))
                    action[i] = act
        ...
        with torch.no_grad():
            ...
            for i in range(args.num_envs):
                next_value[i] = agents[int(next_rm_state[i].item())].get_value(next_obs[i].unsqueeze(0))
        ...
        # Split b_inds based on b_rm_states values
        b_rm_inds = {}
        for rm_state in range(num_states):
            b_rm_inds[rm_state] = b_inds[b_rm_states.cpu().numpy() == rm_state]
        
        for rm_state, agent in enumerate(agents):
            clipfracs = []
            for epoch in range(args.update_epochs):
                np.random.shuffle(b_rm_inds[rm_state])
                for start in range(0, len(b_rm_inds[rm_state]), args.minibatch_size):
                    end = start + args.minibatch_size
                    mb_inds = b_rm_inds[rm_state][start:end]
            ...
            writer.add_scalar(f"charts/learning_rate_{rm_state}", optimizers[rm_state].param_groups[0]["lr"], global_step)
            writer.add_scalar(f"losses/value_loss_{rm_state}", v_loss.item(), global_step)
            writer.add_scalar(f"losses/policy_loss_{rm_state}", pg_loss.item(), global_step)
            writer.add_scalar(f"losses/entropy_{rm_state}", entropy_loss.item(), global_step)
            writer.add_scalar(f"losses/old_approx_kl_{rm_state}", old_approx_kl.item(), global_step)
            writer.add_scalar(f"losses/approx_kl_{rm_state}", approx_kl.item(), global_step)
            writer.add_scalar(f"losses/clipfrac_{rm_state}", np.mean(clipfracs), global_step)
            writer.add_scalar(f"losses/explained_variance_{rm_state}", explained_var, global_step)
            print("SPS:", int(global_step / (time.time() - start_time)))
            writer.add_scalar(f"charts/SPS_{rm_state}", int(global_step / (time.time() - start_time)), global_step)
